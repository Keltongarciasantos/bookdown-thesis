...
Na Figura \@ref(fig:serie31), pode-se identificar claramente uma tendência, o que traduz a não estacionaridade da série. Realizaremos ainda alguns testes para concluirmos sobre a estacionaridade ou não da série.
...
knitr::kable(modeloarima, 'pipe',longtable = TRUE, booktabs = TRUE,
  caption = ' Estimativa do modelo ARIMA(1,2,1).')
```
...
Por definição *outliers* são observações inesperadas que afasta significativamente das observações esperadas e normalmente correspondem a acontecimentos atípicos. A predição da ocorrência futura de *outliers* é muito desafiadora, pois são valores que não se observam frequentemente [@Reunanen2020].  Com o avanço da computacional, existem agora alguns métodos aplicados para o previsão de ocorrência de *outiliers*.  Modelos que usam o *Machine Learning*, têm sido aplicados com o objectivo de prever *outliers*, mas é desafiador também para esses modelos, pois normalmente não existem dados  suficientes para treinar desses modelos de forma a identificar os *ouliers*. Para mais informações sobre métodos de previsão de *outliers* consultar Reunanen [@Reunanen2020] ou Gerogiadis [@georgiadis2013continuous].

# Conclusão

Uma grande parte das observações de fenómenos económico-financeiros podem ser representados em forma de séries temporais. Essas séries, que são caracterizadas por pontos observados com igual espaçamento temporal podem ser analisadas e utilizadas para fazer previsões sobre valores futuros. Existem vários métodos usados na análise e previsão de séries temporais, sendo que a maioria usa valores passados da série para criar um modelo usado para representação e previsão da série.

Dentre os métodos utilizados na análise de séries temporais, destaca-se o $ARIMA(p,d,q)$, que é a combinação dos modelos auto regressivos de ordem $p$ $(AR(p))$ e dos modelos médias-móveis de ordem $q$ $(MA(q))$ onde é aplicado a diferenciação de ordem $d$ na série original para a transformar numa série estacionária. Esse método é muito utilizado na análise de séries económicas e financeiras.

Nesse trabalho, foi utilizado o modelo $ARIMA(1,2,1)$ na análise e previsão do Produto Interno Bruto (PIB) de Cabo Verde com dados de $1980$ a $2018$. A série ajustada obtido produziu valores muito próximos dos valores reais e conseguiu obter previsões muito próximos dos valores reais para o ano 2019. Entretanto a previsão obtida para o ano $2020$ ficou muito longe dos valores efectivos. O ano 2020 foi marcado pela queda da economia mundial devido a pandemia da COVID-19, fazendo com que o PIB da maioria dos países caíssem para valores nunca antes visto. Em 2019 a previsão feita pelo Fundo Monetário Internacional[@Internacional2020] era de que o PIB de Cabo Verde cresceria em torno dos 5% em 2020, entretanto houve uma recessão de 14,8% contrariando todas as previsões feitas, incluíndo as produzidas pelo $ARIMA$.

Um dos pontos fracos do modelo ARIMA e da maioria dos métodos de previsão é justamente a previsão de *outliers* quando não forem frequentemente observadas outros *ouitliers* semelhantes nos dados históricos (como foi o caso do PIB de Cabo Verde em 2020). Para rmitigar esse problema, pode-se... 
