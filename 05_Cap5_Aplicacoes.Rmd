...
Ao aplicar mais uma diferença à nossa série $(d=2)$, e repetir o teste *adf* encontramos um *p-value* $p = 0.01 < 0.05$, ou seja, com um nível de significância de $0.05$, rejeitamos a hipótese inicial de que a nossa série é não estacionária. 
...
Assim o modelo $ARIMA(1,2,1)$ e os seus respetivos parâmetros encontram-se resumidos na tabela abaixo. Podemos também observar que o $p-value$ dos coeficientes $AR(1)$ e $MA(1)$ são pequenos (menores que o nível de significância de $0.05$), ou seja, os coeficientes são significativamente diferentes de zero. 
...
Na Figura \@ref(fig:serie8) encontra-se a representação das funções $ACF$ e $PACF$ dos resíduos. Pode-se ver que não existe autocorrelação dos resíduos, dado que se encontram dentro dos limites de confiabilidade (linha tracejada azul). Além disso, aplicando o teste de *Box-Pierce* ou *Ljung-Box* encontrou-se um $p-value$ muito alto, o que indica que os resíduos comportam-se como ruído branco. Conclui-se então que o nosso modelo é apropriado para fazer as previsões.
...
Dado que a análise dos resíduos mostrou-se adequada a validação do modelo, pode-se agora realizar previsões com base na amostra. Na Figura \@ref(fig:serie9) estão representados os valores efetivos de 1980 a 2018 e os valores obtidos através do nosso modelo no mesmo período. Pode-se observar que o modelo ajustado converge para a série original.
