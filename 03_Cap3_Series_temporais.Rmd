...
Uma pequena análise gráfica, pode dar uma ideia do comportamento dos dados, mas temos sempre que realizar testes estatísticos específicos para conluir sobre determinado comportamento dos dados, nesse caso a tendência. Veremos mais adiante alguns métodos usados para detetar a existência de tendência em séries temporais.
...
Defini-se operador de diferenças por:
```
\begin{equation}
Bx_t = x_{t-1}
(\#eq:operdifer)
...
A **Sazonalidade** é um tipo de variação onde fenómenos que ocorrem durante o tempo se repete a cada período idêntico de tempo. Por exemplo o aumento de vendas de uma loja todos os anos no período de Natal.
